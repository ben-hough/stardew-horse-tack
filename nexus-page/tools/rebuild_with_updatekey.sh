#!/usr/bin/env bash
# Rebuild HorseTack 1.4.0 with the Nexus UpdateKey, on the box.
...
m = json.loads(s2)
assert m["Version"] == "1.4.0", m["Version"]
...
if [ -f "$Z" ] && [ ! -f "$W/share/HorseTack-1.4.0-no-updatekey.zip" ]; then  # keep the pre-UpdateKey zip once
  python3 -c "import zipfile,sys; sys.exit(0 if b'Nexus:' not in zipfile.ZipFile('$Z').read('HorseTack/manifest.json') else 1)" \
    && cp "$Z" "$W/share/HorseTack-1.4.0-no-updatekey.zip" || true
fi
python3 - "$W/pkg" "$Z" <<'PY'
...
SHA=$(sha256sum "$Z" | cut -d' ' -f1); echo "$SHA  HorseTack-1.4.0.zip" > "$Z.sha256"

if [ "$COMMIT" = "--commit" ]; then
  if [ -z "$(git status --porcelain | grep -v ' manifest.json$')" ]; then
    git add manifest.json && git commit -q -m "Add Nexus UpdateKey (Nexus:$ID), version stays 1.4.0" && git push -q && git log --oneline -1
  else
...
SHA-256: $SHA
Next (PC):
  1. CopyFromBox $Z -> C:\\Users\\Glim\\codex-stage\\HorseTack-1.4.0.zip
  2. powershell -ExecutionPolicy Bypass -File E:\\Codex-Mods\\stardew\\HorseTack\\nexus-page\\tools\\install-horsetack.ps1 -NexusId $ID -Sha256 $SHA
MSG
