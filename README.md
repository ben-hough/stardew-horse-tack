...
- **Console commands** (SMAPI window): `horsetack_open [horse]`, `horsetack_list`, `horsetack_options [layer]`, `horsetack_set <horse> <coat|saddle|pad|bridle|style> <id|none>`, `horsetack_reset <horse>`, `horsetack_reload`, `horsetack_textures` (debug: which texture each horse is drawing).
...
Layer order, bottom to top: coat, styling, pad, saddle, bridle. Pads sit under saddles (since 1.4.0), so any pad works with any saddle: Elle's pads no longer paint over HorseTack saddles, and HorseTack's pads no longer need a gap for hers.
...
- **Seasonal horse packs** (Content Patcher packs that change `Animals/horse` by season or condition, e.g. Bog's Witchy Farm Buildings) still change horses on *Keep current*, including their tack (rebuilt on the new art). A chosen HorseTack coat is HorseTack's own texture, built only from HorseTack's pixels, so those packs, `patch reload` and SMAPI's asset propagation can't replace it.
...
- **Seasonal art in multiplayer:** only the option id is synced (e.g. `pads/forest-spirit-moss-cloak`); every computer picks the season file itself from the game's date, which the host shares with everyone, so all players see the same season (a player without the seasonal files sees the plain one). The season is the farm's calendar season, also on Ginger Island.
...
## Credits
- The Forest Spirit set is original art built from generic woodland motifs; it's not affiliated with any other mod and uses nothing from one.
- HorseTack's Breeds coats are inspired by real breeds; not affiliated with Elle's packs. The Witchy set is original art made to suit witchy-themed farms; it's not affiliated with any other mod and uses nothing from one.
...
## Changelog
- **1.4.1**: Chosen coats always win over seasonal horse packs: a coat picked in HorseTack (and its tack) no longer depends on the game's `Animals/horse` texture at all, and every horse is re-applied right after that texture changes (Content Patcher seasonal/conditional packs, `patch reload`, day start; HorseTack's day-start handler now runs after Content Patcher's). *Keep current* horses keep following the live base texture. Fixed: after a horse texture change, horses nobody was looking at (e.g. on the farm while you were indoors) could keep a disposed texture, which the Animals tab then tried to draw. New debug command `horsetack_textures`. No art changes; saved choices keep working.
- **1.4.0**: Forest Spirit collection (8 original pieces: Spirit Coat, Heartwood and Root & Stone saddles, Moss Cloak pad, Glowing Runes and Vine bridles, Antler Crown and Wisp Halo styling; 57 total: 16 coats, 12 saddles, 11 pads, 9 bridles, 9 styling overlays). **Seasonal art**: optional `Name.<season>.png` files are picked from the in-game season (Moss Cloak and Antler Crown have spring/fall/winter trims), same id on every computer. **Pads now draw under saddles**, which fixes Elle's pads painting over HorseTack saddles. Art pass: fringe/drape on all pads (visible from the side and front), Familiar readable on dark coats and visible from the back, real mushrooms on Moss & Mushroom, stronger Red Dun dorsal stripe, outlined Crescent Charm (also on head icons), clearer Prismatic Pearl opal flecks, chunkier Luau Lei, a proper Junimo Buddy, calmer Blue Ribbon + Crown while eating. Saved choices keep working (ids unchanged).
- **1.3.0**: 22 new original pieces (49 total: 15 coats, 10 saddles, 10 pads, 7 bridles, 7 styling overlays), all with `@elle` fits for overlays. New collections: Breeds (Silver Bay, Red Dun, Grulla, Dapple Grey, Brindle, Sabino, Gold Champagne), Prismatic Night/Pearl coats, Ranch (Sage saddle + bridle), Winter (Frostglass saddle + pad), and Witchy (Broomstick and Grimoire saddles, Moss & Mushroom and Starry Hex pads, Potion Vials and Crescent Charm bridles, Witch Hat and Familiar styling, Midnight Familiar coat). No code changes.
