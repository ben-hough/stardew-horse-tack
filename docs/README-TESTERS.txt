Horse Tack & Styling 1.4.0 (test build) by MrGlim
...
3. EVERY player in the multiplayer game must install HorseTack (the host must have it).
4. REMOVE "Multiplayer Horse Reskin" (MultiplayerHorseReskin folder) if you have it.
   It also replaces horse textures, conflicts with HorseTack, and shows missing/blank
   horses without a skin pack. HorseTack warns about it in the SMAPI console.
5. Recommended (optional): Elle's Cuter Horses (needs Content Patcher)
   https://www.nexusmods.com/stardewvalley/mods/20042
   Its 79 coats, prismatic hair and 24-colour saddles/pads/bridles then show up in the
   wizard next to HorseTack's own art. HorseTack's own art works with or without it.
...
saddle), bridle, styling, confirm. Left/Right (or D-pad left/right, or the arrows)
...
Please report: anything that looks misaligned (which horse coat + which piece), and
whether farmhands see the same look as the host.

Source: https://github.com/ben-hough/stardew-horse-tack
AI disclosure: this mod's code and its bundled art were made with AI assistance.
